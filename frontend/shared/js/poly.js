...
(function templatePolyfill(d) {
	if('content' in d.createElement('template')) return false;
	
	var templates = d.getElementsByTagName('template');
	var iter = templates.length;
	while(iter-->0)
	{
		var template = templates[iter];
		var nodes = template.childNodes;
		var content = d.createDocumentFragment();
		while(nodes[0]) content.appendChild(content[0]);
